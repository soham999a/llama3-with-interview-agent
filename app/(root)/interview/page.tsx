    ...
    return (
      <div className="flex flex-col gap-8">
        <div className="flex justify-between items-center">
          <h1 className="text-teal-600 text-3xl font-bold">New Interview</h1>
          <div className="flex items-center gap-4">
            <Button asChild className="bg-white text-teal-600 border border-teal-200 px-4 py-2 rounded-lg hover:bg-teal-50 transition-all">
        ...
        <div className="flex flex-col items-center justify-center h-64 gap-4">
          ...
          <p className="text-gray-600 font-medium">Loading interview options...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex justify-between items-center">
        <h1 className="text-teal-600 text-3xl font-bold">New Interview</h1>
        <div className="flex items-center gap-4">
          <Button onClick={handleBack} className="bg-white text-teal-600 border border-teal-200 px-4 py-2 rounded-lg hover:bg-teal-50 transition-all">
            {step === 1 ? 'Cancel' : 'Back'}
          </Button>
        </div>
      </div>
      ...
      <div className="relative bg-white rounded-xl p-6 border border-gray-200 shadow-md mb-8">
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div className="flex flex-col items-center gap-2">
            <div className={`rounded-full size-10 flex items-center justify-center shadow-md ${
              step >= 1
            ...
            <span className={`font-medium ${step >= 1 ? 'text-gray-800' : 'text-gray-400'}`}>Type</span>
          </div>

          <div className="relative">
            <div className={`h-1 w-24 flex-shrink-0 rounded-full ${step >= 2 ? 'bg-teal-500' : 'bg-gray-200'}`}></div>
            {step >= 2 && (
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <span className={`font-medium ${step >= 2 ? 'text-gray-800' : 'text-gray-400'}`}>Role</span>
          </div>

          <div className="relative">
            <div className={`h-1 w-24 flex-shrink-0 rounded-full ${step >= 3 ? 'bg-teal-500' : 'bg-gray-200'}`}></div>
            {step >= 3 && (
              <div className="absolute top-0 left-0 h-1 w-24 rounded-full bg-teal-300 opacity-50 animate-pulse"></div>
            )}
          </div>
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <span className={`font-medium ${step >= 3 ? 'text-gray-800' : 'text-gray-400'}`}>Tech Stack</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          {step === 1 && (
            <>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-semibold text-gray-800">Select Interview Type</h2>
              </div>
                ...
                {interviewTypes.map((option) => (
                  ...
                  >
                    ...
                    <div className={`rounded-xl ${
                      ...
                      <Image src={option.icon} alt={option.title} width={28} height={28} className={
                        option.id === 'technical' ? 'text-teal-600' :
                        option.id === 'behavioral' ? 'text-blue-600' :
                        'text-green-600'
                      } />
                    </div>
          ...
          {step === 2 && (
            <>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-semibold text-gray-800">Select Role</h2>
              </div>
              <p className="text-gray-600 mb-6">Choose the role you want to practice interviewing for</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {roleOptions.map((role) => (
                  ...
                  >
                    ...
                    <div className={`rounded-full bg-blue-100 p-3 relative z-10 shadow-md ${selectedRole === role ? 'animate-pulse-slow' : ''}`}>
          ...
          {step === 3 && (
            <>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-semibold text-gray-800">Select Tech Stack</h2>
              </div>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {selectedTechStack.length > 0 ? (
                    selectedTechStack.map((tech) => (
                      <div key={tech} className="bg-green-100 text-green-600 px-3 py-1.5 rounded-full text-sm font-medium flex items-center gap-1">
                        {tech}
                        <button
                ...
                {techStackOptions.map((tech) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 relative z-10">
                      ...
                      <span className={`font-medium ${selectedTechStack.includes(tech) ? 'text-green-600' : 'text-gray-800'}`}>{tech}</span>
                    </div>
                  </button>
                ))}
              </div>
        ...
        <div className="relative overflow-hidden bg-white rounded-xl p-8 border border-gray-200 shadow-md flex flex-col h-full">
          ...
          <div className="relative z-10 flex flex-col items-center justify-center text-center h-full">
            ...
            <h3 className="text-xl font-semibold text-gray-800 mb-3">AI Interview Assistant</h3>
            <div className="bg-teal-100 text-teal-600 px-3 py-1 rounded-full text-sm font-medium mb-6">
              Step {step} of 3
            </div>

            {step === 1 && (
              <p className="text-gray-600 mb-6">Select an interview type to begin your practice session</p>
            )}
            {step === 2 && (
              <p className="text-gray-600 mb-6">Choose the role you want to practice interviewing for</p>
            )}
            {step === 3 && (
              <p className="text-gray-600 mb-6">Select the technologies you're familiar with for your interview</p>
            )}
              ...
              <div className="mt-4 w-full bg-white rounded-xl p-6 text-left border border-gray-200 shadow-md">
                  ...
                  {selectedType && (
                    <li className="flex items-start gap-3">
                      <div className={`rounded-lg p-2 ${
                        selectedType === 'technical' ? 'bg-teal-100' :
                        selectedType === 'behavioral' ? 'bg-blue-100' :
                        'bg-green-100'
                      }`}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={
                          selectedType === 'technical' ? 'text-teal-600' :
                          selectedType === 'behavioral' ? 'text-blue-600' :
                          'text-green-600'
                        }>
                          <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>
                        </svg>
                      ...
                      <div>
                        ...
                        <p className="text-gray-800 font-medium">{interviewTypes.find(t => t.id === selectedType)?.title}</p>
                      </div>
                    </li>
                  )}
                  {selectedRole && (
                    <li className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-800 font-medium">{selectedRole}</p>
                      </div>
                    </li>
                  )}
                  {selectedTechStack.length > 0 && (
                    <li className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-1">
                          {selectedTechStack.map(tech => (
                            <span key={tech} className="bg-green-100 text-green-600 px-2 py-0.5 rounded-full text-xs font-medium">
                              {tech}
                            </span>
                          ))}
                        </div>
                ...
                {step === 3 && selectedType && selectedRole && selectedTechStack.length > 0 && (
                  <div className="mt-6 pt-4 border-t border-gray-200">
                    <p className="text-gray-600 text-sm">Your interview is ready! Click "Continue to Interview" to start your practice session.</p>
                  </div>
                )}
              </div>
            )}
