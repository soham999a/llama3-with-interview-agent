  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-teal-600">LLAMA3 INTERVIEW DASHBOARD</h1>
        <div className="flex items-center gap-4">
          <Button asChild className="bg-white text-teal-600 border border-teal-200 px-4 py-2 rounded-[1.25rem] hover:bg-teal-50 transition-all">
      ...
      <div className="bg-white rounded-[2rem] p-8 border border-gray-200 shadow-md">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          ...
          <div className="mt-4 md:mt-0 flex items-center gap-2 bg-teal-100 px-4 py-2 rounded-[1.25rem] shadow-sm">
            ...
            <span className="text-teal-600 font-bold">{feedback?.totalScore || 0}</span>
        ...
        <div className="border-t border-gray-200 pt-6 mt-2">
          ...
          <p className="text-gray-600 bg-gray-50 p-4 rounded-[1.25rem] border-l-4 border-teal-500">{feedback?.finalAssessment}</p>
        </div>
        ...
        <div className="border-t border-gray-200 pt-6 mt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {feedback?.categoryScores?.map((category, index) => (
              <div key={index} className="bg-white rounded-[1.25rem] p-5 border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex justify-between items-center mb-2">
                  <h4 className="font-medium text-gray-800">{category.name}</h4>
                  <div className="flex items-center gap-1 bg-teal-100 px-3 py-1 rounded-full">
                    <span className="text-teal-600 font-bold">{category.score}</span>
          ...
          <div className="bg-white rounded-[1.25rem] p-5 border border-gray-200 shadow-sm">
            ...
            <ul className="space-y-3">
              {feedback?.strengths?.map((strength, index) => (
                <li key={index} className="flex items-start gap-2 text-gray-600 bg-green-50 p-2 rounded-lg">
          ...
          <div className="bg-white rounded-[1.25rem] p-5 border border-gray-200 shadow-sm">
            ...
            <ul className="space-y-3">
              {feedback?.areasForImprovement?.map((area, index) => (
                <li key={index} className="flex items-start gap-2 text-gray-600 bg-orange-50 p-2 rounded-lg">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mt-8">
          <Button asChild className="bg-white text-teal-600 border border-teal-200 px-6 py-3 rounded-[1.25rem] font-medium transition-all duration-300 hover:bg-teal-50 hover:shadow-teal-100 hover:shadow-lg shadow-md flex-1 hover:-translate-y-1 active:scale-95">
          ...
          <Button asChild className="bg-teal-500 text-white px-6 py-3 rounded-[1.25rem] font-medium transition-all duration-300 hover:opacity-90 hover:shadow-teal-200 hover:shadow-lg shadow-md flex-1 hover:-translate-y-1 active:scale-95">
