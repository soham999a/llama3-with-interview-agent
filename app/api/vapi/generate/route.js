...
export async function POST(request) {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if (Array.isArray(parsedQuestions) && parsedQuestions.length > 0) {
          ...
          console.log(
            "Parsed JSON is not a valid array or is empty, using fallback questions"
          );
        }
      } catch (parseError) {
    ...
    if (questions.length < 5) {
      const additionalQuestions = defaultQuestions.slice(
        0,
        5 - questions.length
      );
      questions = [...questions, ...additionalQuestions];
      console.log(
        "Added additional questions to reach minimum of 5:",
        questions
      );
    }
    ...
    const interview = {
      ...
      questions: questions,
      userId: userid || "anonymous", // Provide a default value if userid is undefined
      finalized: true,
      coverImage: getRandomInterviewCover(),
      createdAt: new Date().toISOString(),
      fixtures: {
        roles: [role], // Add the role to fixtures.roles array
      },
    };
