...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const mockInterviews = generateMockInterviews(userId, 10);
    
    return NextResponse.json({ 
      success: true, 
      interviews: mockInterviews,
      message: `Generated ${mockInterviews.length} mock interviews for testing. Use these in your history page.`
    });
  } catch (error) {
