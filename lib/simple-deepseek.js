...
import { huggingfaceAdapter } from './huggingface-integration';
...
const adapter = isProduction ? huggingfaceAdapter : null;

/**
 * Simple DeepSeek Adapter
 * Uses Hugging Face in production and local Ollama in development
 */
const simpleDeepseekAdapter = {
  ...
  async generateText({ prompt }) {
    // Use Hugging Face adapter in production
    if (isProduction && adapter) {
      return adapter.generateText({ prompt });
    }
  ...
  async generateObject({ prompt }) {
    // Use Hugging Face adapter in production
    if (isProduction && adapter) {
      return adapter.generateObject({ prompt });
    }
